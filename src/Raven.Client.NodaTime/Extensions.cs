...
public static class Extensions
{
    public static T ConfigureForNodaTime<T>(this T documentStore)
        where T : IDocumentStore => documentStore.ConfigureForNodaTime(DateTimeZoneProviders.Tzdb);
    ...
    {
        if (conventions.Serialization is not NewtonsoftJsonSerializationConventions jsonSerializationConventions)
        {
    ...
    public static TimeSpan ToTimeSpan(this LocalTime localTime) => new(localTime.TickOfDay);

    public static LocalTime ToLocalTime(this TimeSpan timeSpan) => LocalTime.FromTicksSinceMidnight(timeSpan.Ticks);
}
